...
public interface TomatoPredictRepository extends JpaRepository<TomatoPredict, Long> {
    ...
    @Query("SELECT c FROM TomatoPredict c WHERE c.grade = :grade ORDER BY c.year DESC, c.month DESC, c.day DESC LIMIT 1")
    TomatoPredict findLatestByGrade(@Param("grade") Grade grade);

    @Query("SELECT c FROM TomatoPredict c WHERE (c.year > :fromYear OR (c.year = :fromYear AND c.month > :fromMonth) OR (c.year = :fromYear AND c.month = :fromMonth AND c.day >= :fromDay)) " +
    ...
    List<TomatoPredict> findByDateRangeAndGrade(int fromYear, int fromMonth, int fromDay,
