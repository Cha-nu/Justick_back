...
public class PotatoService {
    ...
    // 최근 21일 데이터 조회
    public List<Potato> findRecentDaysByGrade(Grade grade) {
        Pageable top21 = PageRequest.of(0, 21); // 상위 21개
        List<Potato> recent = repository.findTopByGradeOrderByDateDesc(grade, top21);
        Collections.reverse(recent); // 날짜 오름차순 정렬로 바꿔줌 (old → new)
        return recent;
    }

    public Map<String, Map<String, Integer>> getWeeklyAverages(Grade grade) {
        List<Potato> all = repository.findByGrade(grade);
    ...
    public Map<String, Map<String, Integer>> getMonthlyAverages(Grade grade) {
        ...
        Map<String, Map<String, Integer>> fullResult = all.stream()
                ...
                .collect(Collectors.groupingBy(
                        ...
                        Collectors.collectingAndThen(Collectors.toList(), list -> {
                            int avgPrice = (int) list.stream().mapToInt(Potato::getAveragePrice).average().orElse(0);
                            int avgIntake = (int) list.stream().mapToInt(Potato::getIntake).average().orElse(0);
                            Map<String, Integer> m = new HashMap<>();
                            m.put("averagePrice", avgPrice);
                            m.put("intake", avgIntake);
                            return m;
    ...
    public void saveAll(List<PotatoRequest> requests) {
        List<Potato> list = requests.stream().map(this::toEntity).toList();
        repository.saveAll(list);
    }

    private Potato toEntity(PotatoRequest request) {
        Potato entity = new Potato();
    ...
    @Transactional
    public void saveOneAndDeleteOldest(PotatoRequest request) {
        Grade grade = Grade.valueOf(request.getGrade());
        long count = repository.countByGrade(grade);
        if (count >= 1) {
            // 가장 오래된 데이터 id 조회
            Optional<Potato> oldest = repository.findByGradeOrderByYearAscMonthAscDayAsc(grade).stream().findFirst();
