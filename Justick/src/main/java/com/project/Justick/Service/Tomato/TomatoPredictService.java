...
@Service
public class TomatoPredictService {

    private final TomatoPredictRepository repo;
    ...
    public Map<String, Map<String, Integer>> getWeeklyAverages(Grade grade) {
        List<TomatoPredict> all = repo.findByGrade(grade);
        ...
        Map<String, Map<String, Integer>> grouped = all.stream()
                ...
                .collect(Collectors.groupingBy(
                        ...
                        Collectors.collectingAndThen(Collectors.toList(), list -> {
                            int avgPrice = (int) list.stream().mapToInt(TomatoPredict::getAveragePrice).average().orElse(0);
    ...
    public List<TomatoPredict> findRecentDaysWithForecast(Grade grade) {
        TomatoPredict latest = repo.findLatestByGrade(grade);
        ...
        List<TomatoPredict> result = repo.findByDateRangeAndGrade(
    ...
    @Transactional
    public void saveOneAndDeleteOldest(TomatoPredictRequest request) {
        Grade grade = Grade.valueOf(request.getGrade().toUpperCase());
        long count = repo.countByGrade(grade);
        if (count >= 1) {
            // 가장 오래된 데이터 삭제
            repo.findByGrade(grade).stream()
                    .min(Comparator.comparing(TomatoPredict::getYear)
                            .thenComparing(TomatoPredict::getMonth)
                            .thenComparing(TomatoPredict::getDay)).ifPresent(repo::delete);
        }
